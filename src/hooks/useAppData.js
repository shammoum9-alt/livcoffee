...
export function useAppData() {
  ...
  const setProduits = useCallback((updater) => {
    setProduitsState(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      const idsAvant = new Set(prev.map(p => p.id));
      const idsApres = new Set(next.map(p => p.id));
      const supprimes = [...idsAvant].filter(id => !idsApres.has(id));
      supprimes.forEach(id => {
        data.deleteProduit(id).catch(err => reportWriteError("suppression de recette", err));
      });
      if (next.length > 0) {
  ...
  // ── Référentiel CBC/CBD ──
  const setCbcData = useCallback((updater) => {
    setCbcDataState(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      const idsAvant = new Set(prev.map(c => c.id));
      const idsApres = new Set(next.map(c => c.id));
      const supprimes = [...idsAvant].filter(id => !idsApres.has(id));
      supprimes.forEach(id => {
        data.deleteCbcDataProduit(id).catch(err => reportWriteError("suppression produit CBC/CBD", err));
      });
      if (next.length > 0) {
        data.upsertCbcDataBatch(next).catch(err => reportWriteError("produit CBC/CBD", err));
      }
      return next;
    });
  }, []);
