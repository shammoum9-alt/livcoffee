...
const NOUVEAU_PRODUIT_VIDE = (categorie) => ({
  id: "produit_" + Date.now(),
  nom: "Nouveau produit",
  prix: 0,
...
export default function Recettes({ produits, setProduits, nbres, achats }) {
  ...
  const aUneRecette = p && p.ingredients.length > 0;
  ...
  const ajouterProduit = (categorie) => {
    const nouveau = NOUVEAU_PRODUIT_VIDE(categorie);
    setProduits(prev => [...prev, nouveau]);
    setSelected(nouveau.id);
    setEditData(JSON.parse(JSON.stringify(nouveau)));
    setEditing(true);
  ...
  const catsDisponibles = ["sam", "sucre", "sandwich", "boisson"];
      ...
      <div>
        <div style={{ marginBottom: "1rem", position: "relative" }}>
          <button onClick={() => setShowAddMenu(!showAddMenu)} style={{
            width: "100%", padding: "8px 12px", border: "0.5px dashed var(--color-border-tertiary)",
            borderRadius: "var(--border-radius-md)", background: "none", cursor: "pointer",
          ...
          {showAddMenu && (
            <div style={{
              ...
              borderRadius: "var(--border-radius-md)", boxShadow: "0 4px 16px rgba(0,0,0,0.1)", overflow: "hidden",
            }}>
              {catsDisponibles.map(cat => (
                <button key={cat} onClick={() => ajouterProduit(cat)} style={{
                  display: "block", width: "100%", textAlign: "left", padding: "8px 12px",
                  border: "none", background: "none", cursor: "pointer", fontSize: 13, color: "var(--color-text-primary)",
                }}>
                  {CAT_LABELS[cat] || cat}
                </button>
              ))}
            ...
            {produits.filter(x => x.categorie === cat).map(prod => (
              <button key={prod.id} onClick={() => { setSelected(prod.id); setEditing(false); }} style={{
                ...
                {prod.ingredients.length > 0 ? (
                  ...
                  <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 2, fontStyle: "italic" }}>
                    Sans recette détaillée
                  </div>
                )}
              </button>
            ))}
        ...
        <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1.25rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={supprimerProduit} style={{ border: "0.5px solid rgba(220,53,69,0.4)", background: "rgba(220,53,69,0.06)", cursor: "pointer", padding: "6px 12px", borderRadius: "var(--border-radius-md)", fontSize: 13, color: "#dc3545" }}>
                Supprimer
              </button>
            </div>
          </div>
          ...
          {!editing ? (
            <>
              {aUneRecette ? (
                ...
                <div style={{ padding: "1.5rem", textAlign: "center", color: "var(--color-text-secondary)", fontSize: 13, background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)" }}>
                  Produit sans recette détaillée — prix de vente fixe, pas de calcul de coût de revient ni de suivi de stock d'ingrédients.
                  <div style={{ marginTop: 8, fontSize: 20, fontWeight: 600, color: "var(--color-text-primary)" }}>{fmtE(p.prix)}</div>
                </div>
              )}
            ...
            <div>
              <div style={{ marginBottom: "1rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <div>
                  <label style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Nom du produit</label>
                ...
                <div>
                  <label style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Catégorie</label>
                  <select value={editData.categorie} onChange={e => setEditData({ ...editData, categorie: e.target.value })}
                    style={{ display: "block", width: "100%", marginTop: 4, padding: "6px 8px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", background: "var(--color-background-primary)", color: "var(--color-text-primary)", fontSize: 13 }}>
                    {["sam", "sucre", "sandwich", "boisson"].map(c => (
              ...
              <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: 4 }}>
                Ingrédients <span style={{ fontStyle: "italic" }}>(facultatif — laisse vide pour un produit sans recette détaillée, ex: boisson fraîche)</span>
              </div>
